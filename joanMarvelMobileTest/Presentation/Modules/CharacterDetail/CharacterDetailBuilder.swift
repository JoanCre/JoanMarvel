...
final class CharacterDetailBuilder {
    func build(characterID: Int) -> CharacterDetailViewController {
        guard let viewController = UIStoryboard(name: "CharacterDetailView", bundle: nil).instantiateViewController(withIdentifier: "CharacterDetailView") as? CharacterDetailViewController
            else {
            fatalError("Error instantiate CharacterDetailViewController")
